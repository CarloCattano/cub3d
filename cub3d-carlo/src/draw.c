...
/*   Updated: 2023/11/06 13:30:38 by carlo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	ft_bzero(d->img.pix, WIDTH * HEIGHT * (d->img.bpp >> 3));
}

static void calculate_step_and_side_distances(t_ray *ray, t_data *d) {
    if (ray->rayDirX < 0) {
        ray->stepX = -1;
...
static char wall_orientation(double rayAngle)
{
    char Direction;
